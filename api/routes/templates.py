...
from ..models.templates.templates_versions import TemplatesVersions
...
@router.get(
    "/versions/available",
    response_model=TemplatesVersions,
    operation_id="get_template_versions",
    description="""Retrieve the available versions of codex templates.
This endpoint returns a structured response with template versions. The versions are sorted in descending order so that the highest version appears first.""",
    summary="Get available template versions",
)
async def get_versions_available(
    session: DescopeSession = Depends(get_descope_session),
):
    """Retrieve the available versions of codex templates.

    Raises:
        HTTPException: - 401 UNAUTHORIZED: If no valid session is provided.
        HTTPException: - 403 FORBIDDEN: If the session does not have sufficient scopes.

    Returns:
        TemplatesVersions: A response object containing the available template types and their versions.
    """
    if session:
        if not session.scopes.intersection(_TEMPLATE_SCOPE.read_scopes):
            logger.error("Insufficient scope to access template status.")
...
@router.get(
    ...
    description="""Retrieve the available types of codex templates.
This endpoint returns a list of template types.""",
    summary="Get available template types",
)
async def get_types_available(
    session: DescopeSession = Depends(get_descope_session),
) -> list[str]:
    """Retrieve the available types of codex templates.
    ...
    Returns:
        list: A list of available template types.
    """
    if session:
        if not session.scopes.intersection(_TEMPLATE_SCOPE.read_scopes):
            logger.error("Insufficient scope to access template status.")
