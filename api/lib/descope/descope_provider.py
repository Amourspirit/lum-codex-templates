...
from fastmcp.server.auth.providers.descope import DescopeProviderSettings
from api.lib.descope.auth_config import get_settings

# This provider is a replacement for the one in fastmcp.
# In the current version of fastmcp, Version 2.14.4, the DescopeProvider is broken.
# The init uses `agentic` hardcoded in the URL path, to find the project ID.
# This is no longer valid, as Descope has changed their URL structure.
...
class DescopeProvider(RemoteAuthProvider):
    ...
    def __init__(
        ...
        *,
        ...
        """
        self._auth_settings = get_settings()
        ...
        if settings.config_url is not None:
            ...
            self.project_id = self._auth_settings.DESCOPE_PROJECT_ID
            ...
            logger.error(
                "DescopeProvider initialization error: insufficient configuration."
            )
            raise ValueError(
        ...
        logger.debug("DescopeProvider initialized successfully.")
    ...
    def get_routes(
        ...
        async def oauth_authorization_server_metadata(request):
            ...
            try:
                ...
                logger.error(
                    "Failed to fetch Descope OAuth authorization server metadata: {error}",
                    error=e,
                )
                return JSONResponse(
                    {
