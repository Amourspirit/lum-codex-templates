...
class PreProcessorNodeLinkScroll(
    ...
    def _get_file_name(self) -> str:
        return "process_node_link_scroll_template.py"
...
class ProcessNodeLinkScrollTemplate:
    ...
    def _format(self) -> FrontMatterMeta:
        # Process and format the node_link_scroll template content
        ...
        if fm.has_field("artifact_activator"):
            fm.frontmatter["artifact_activator"] = [self.monad_name]
        keys = {"source_medium"}
        for key in keys:
    ...
    def render(self) -> str:
        # Render the node_link_scroll template with the provided context
